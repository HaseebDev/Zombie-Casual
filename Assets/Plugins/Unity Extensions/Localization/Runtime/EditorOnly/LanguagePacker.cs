...
namespace UnityExtensions.Localization.Editor
{
    public static class LanguagePacker
    {
        ...
        public const string targetFolder = "Assets/Resources/Localization";
        ...
        public static bool ReadExcels()
        {
            try
            {
                ...
                if (fileCount == 0)
                {
                    ...
                    File.Copy(Path.GetFullPath("Assets/Plugins/Unity Extensions/Localization/Runtime/EditorOnly/EditorResources/Sample.xlsx"), filePath, false);
                    ReadExcel(filePath);
                }
        ...
        public static bool WritePacks()
        {
            try
            {
                ...
                using (var stream = new FileStream($"{targetFolder}/{metaFileName}.txt", FileMode.Create, FileAccess.Write))
                {
                ...
                foreach (var lang in _languageTexts)
                {
                    using (var stream = new FileStream($"{targetFolder}/{lang.Key}.txt", FileMode.Create, FileAccess.Write))
                    {
                        using (var writer = new BinaryWriter(stream))
                        {
                            // texts
