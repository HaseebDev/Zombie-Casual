...
public class IAPManager : BaseSystem<IAPManager>, IStoreListener, IMyIAPManager
{
    ...
    public void InitializePurchasing()
    {
        ...
        foreach (var item in ResourceManager.instance._iAPManagerSO._listIAPProducts)
        {
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        if (m_StoreController != null && m_StoreController.products != null)
        {
            ...
            // Check bough remove ads
            Product product = m_StoreController.products.WithID(IAPConstant.remove_ads);
    ...
    public void OnInitializeFailed(InitializationFailureReason error, string? x)
    {
        // Purchasing set-up has not succeeded. Check error for reason. Consider sharing this reason with the user.
        Debug.Log("OnInitializeFailed InitializationFailureReason:" + error);
        Debug.Log("OnInitializeFailed InitializationFailure string:" + x);
    }
    ...
    private void OnPurchaseSuccess(string id)
    {
        ...
        AnalyticsManager.instance.LogEvent(AnalyticsConstant.ANALYTICS_ENUM.IAP_PURCHASE_SUCCESS,
            new LogEventParam("productId", id));
    }
    ...
    public void OnPurchaseFailed(Product product, PurchaseFailureReason failureReason)
    {
        ...
        AnalyticsManager.instance.LogEvent(AnalyticsConstant.ANALYTICS_ENUM.IAP_PURCHASE_FAIL,
            new LogEventParam("productId", product.definition.id));
    }
    ...
    {
        if (m_StoreController != null && m_StoreController.products != null &&
            m_StoreController.products.WithID(productID) != null)
            return m_StoreController.products.WithID(productID).metadata.localizedPriceString;
        else
            return GameConstant.ERROR_IAP_COST;
    }

    private Dictionary<string, CoroutineHandle> LoadIAPCoroutine;


    public void GetProductPrice(string productID, Action<string> callback)
    {
        if (LoadIAPCoroutine == null)
            LoadIAPCoroutine = new Dictionary<string, CoroutineHandle>();

        if (!LoadIAPCoroutine.ContainsKey(productID))
        {
            LoadIAPCoroutine.Add(productID, Timing.RunCoroutine(GetProductPriceCoroutine(productID, callback)));
        }
        else
        {
            Timing.KillCoroutines(LoadIAPCoroutine[productID]);
            LoadIAPCoroutine[productID] = Timing.RunCoroutine(GetProductPriceCoroutine(productID, callback));
        }
    }

    private IEnumerator<float> GetProductPriceCoroutine(string productID, Action<string> callback)
    {
        int counter = 0;
        // Skip wait after 2second
        int totalWait = 2;

        while (m_StoreController?.products?.WithID(productID) == null
               && counter < totalWait)
        {
        ...
        if (counter >= totalWait)
        {
            callback?.Invoke(GameConstant.ERROR_IAP_COST);
        }
        else
        {
            callback?.Invoke(m_StoreController.products.WithID(productID).metadata.localizedPriceString);
        }
    }
    ...
    public void PurchaseIAP(string productID, Action<bool> callback)
    {
        ...
        else
        {
            ...
            BuyProductID(productID);
                ...
                AnalyticsManager.instance.LogEvent(AnalyticsConstant.ANALYTICS_ENUM.IAP_PURCHASE_SUCCESS,
          new LogEventParam("productId", productID));
            }
        });
