...
namespace UnityEngine.UI.Extensions
{
	...
	public class AutoCompleteComboBox : MonoBehaviour
	{
		...
		{
			List<string> second = (from x in this._panelItems
			where x.ToLower().Contains(currText.ToLower())
			select x).ToList<string>();
			foreach (string text in this._panelItems.Except(second).ToList<string>())
			{
			...
			where x.ToLower().Contains(currText)
			select x).ToList<string>())
			{
