...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour
    {
        ...
        public void Trigger(Vector3 _startPos, Vector3 _endPos, float duration = 0.3f, Action OnComplete = null)
        {
            Vector3 start, end;
            timer = Duration + Mathf.Min(0.0f, timer);
            StartObject.transform.position = _startPos;
            EndObject.transform.position = _startPos;
            start = StartObject.transform.position + StartPosition;
            end = EndObject.transform.position + EndPosition;
            startIndex = 0;
            GenerateLightningBolt(start, end, Generations, Generations, 0.0f);
            ...
            EndObject.transform.DOMove(_endPos, duration * 0.6f).SetEase(Ease.Linear);
        ...
        public void PlayAnim(Vector3 _startPos, Vector3 _endPos, float duration = 0.3f, Action OnComplete = null)
        {
            EndObject.transform.DOKill();
            StartObject.transform.position = _startPos;
            EndObject.transform.position = _endPos;

            StartObject.transform.DOMove(_endPos, duration * 0.2f).SetEase(Ease.Linear).SetDelay(duration * 0.8f);
            Timing.CallDelayed(duration, () =>
            {
                OnComplete?.Invoke();
            });
