    ...
    public class CellGroupViewsHolder<TCellVH> : BaseItemViewsHolder where TCellVH : CellViewsHolder, new()
    {
        ...
        internal void Init(GameObject groupPrefab, RectTransform parent, int itemIndex, RectTransform cellPrefab, int numCellsPerGroup)
        {
			...
			for (int i = 0; i < _Capacity; ++i)
			{
				...
				if (Pooly.IsContainKey(cellPrefab.name))
				{
					cellInstance = Pooly.Spawn<RectTransform>(cellPrefab.name,Vector3.zero,Quaternion.identity, root);
					cellInstance.localScale = Vector3.one;
				}
				else
				{
					cellInstance = (GameObject.Instantiate(cellPrefab.gameObject, root, false) as GameObject).transform as RectTransform;
				}
